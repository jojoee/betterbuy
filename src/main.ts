import './style.css';
import { compare, formatNumber, type ComparisonInput, type ComparisonResult } from './calculator';
import { deleteHistory, readHistory, saveHistory, type HistoryEntry } from './history';

const emptyInput: ComparisonInput = { costA: NaN, sizeA: NaN, costB: NaN, sizeB: NaN };
...
function inputField(label: 'Cost' | 'Size', option: 'A' | 'B', key: keyof ComparisonInput): string {
...
function resultMarkup(result: ComparisonResult | null): string {
  if (!result) return '<p class="result-placeholder" aria-live="polite">Enter positive Cost and Size values to compare.</p>';
  const headline = result.winner ? `${result.winner} is ${formatNumber(result.savingPercent, 1)}% cheaper` : 'A and B cost the same per size';
...
function historyMarkup(): string {
  if (!history.length) return '<p class="empty-history">No saved comparisons yet.</p>';
  return `<ul class="history-list">${history.map((entry) => {
    const winner = entry.result.winner ? `${entry.result.winner} is ${formatNumber(entry.result.savingPercent, 1)}% cheaper` : 'Same cost per size';
